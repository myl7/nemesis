...
pub extern crate group_math as group;
...
pub use fss_types::{decl_prg_trait, Cw, Share};
    ...
    fn eval(&self, b: bool, k: &Share<LAMBDA, G>, xs: &[&[u8; N]], ys: &mut [&mut G]) {
        ...
        let f = |x: &[u8; N], v: &mut G| {
            ...
            for i in 1..n + 1 {
                ...
                if d != c {
                    d = d ^ ts[i - 1];
                    xor_inplace(&mut ss[i], &[if ts[i - 1] { &[1; LAMBDA] } else { &[0; LAMBDA] }]);
                }
            }
            if d {
                xor_inplace(&mut ss[n], &[if ts[n] { &[1; LAMBDA] } else { &[0; LAMBDA] }]);
            }
