...
// __device__ uint128_t_gpu neg_uint128(uint128_t_gpu a){
//   uint128_t_gpu one = {0};
//   one.x = 1;
...
// uint64_t brev_cpu(uint64_t x){
//   uint64_t s = sizeof(uint64_t) * 8;
...
__device__
RingElement gpu_brev(RingElement x){
  RingElement s = sizeof(RingElement) * 8;
  RingElement mask = ~((RingElement)0);
  while((s>>=1)>0){
    mask ^= (mask<<s);
    x = ((x>>s)& mask)|((x<<s)& ~mask);
  }
...
RingElement cpu_brev(RingElement x){
  RingElement s = sizeof(RingElement) * 8;
  RingElement mask = ~((RingElement)0);
  while((s>>=1)>0){
    mask ^= (mask<<s);
...
__device__
void gpu_xor128(const u_int8_t* a,const u_int8_t* b,u_int8_t* c){
  for(int i = 0; i < 16; ++i){
    c[i] = a[i] ^ b[i];
  }
}

void cpu_xor128(const u_int8_t* a,const u_int8_t* b,u_int8_t* c){
  for(int i = 0; i < 16; ++i){
    c[i] = a[i] ^ b[i];
  }
}
